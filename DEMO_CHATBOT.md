...
| **REFINING** | Tùy chọn — thu thập tiêu chí nâng cao (khu vực cụ thể, tiện ích, loại phòng). Gồm 2 sub-step: *collect* → *confirm*. |
...
| **Bot trả lời** | *"Ạ, mình chỉ hỗ trợ tìm phòng trọ thôi ạ. Bạn có thể mô tả nhu cầu như: ngân sách, khu vực muốn ở, tiện ích cần có — mình sẽ tìm giúp bạn ngay!"* |
...
| **Bot trả lời** | *(Gemini sinh câu hỏi làm rõ, ví dụ:)* *"Bạn muốn tìm phòng ở khu vực nào, và ngân sách khoảng bao nhiêu ạ?"* + chips `[Căn hộ][KTX][Nhà trọ][Nhà nguyên căn]` |
...
| **Bot trả lời** | *"Ngân sách dự kiến của bạn khoảng bao nhiêu một tháng ạ?"* <br> + chips `[Dưới 2 triệu/tháng]` `[2 – 4 triệu/tháng]` `[4 – 7 triệu/tháng]` `[7 – 15 triệu/tháng]` |
...
| **Bot trả lời** | *"Bạn muốn tìm phòng ở tỉnh/thành nào ạ? (ví dụ: Hồ Chí Minh, Hà Nội)"* <br> + chips `[Hồ Chí Minh]` `[Hà Nội]` `[Đà Nẵng]` |
...
|---|---|
| **User gõ** | `Hồ Chí Minh` |
| **Bot trả lời** | *"Mình tìm thấy khoảng **30+ phòng** ở Hồ Chí Minh trong ngân sách của bạn ạ. Để tìm chính xác hơn, bạn muốn ở khu vực/phường cụ thể nào không? Và bạn cần tiện ích gì (máy lạnh, wifi, giường nệm...) hoặc muốn gần chợ, công viên, trường học không ạ?"* <br> + chip `[Tìm ngay luôn]` |
...
| **Bot trả lời** | Summary confirm: *"Mình sẽ tìm **nhà trọ** tại **Hồ Chí Minh**, khu **Tăng Nhơn Phú**, ngân sách **4 triệu/tháng**, có: **Wifi, Điều hòa**. Tìm ngay nhé?"* <br> + chips `[Tìm ngay]` `[Sửa lại]` |
...
| **Bot trả lời** | *"Bạn muốn thay đổi điều gì? Hãy nói thêm về khu vực, tiện ích hoặc ngân sách ạ."* <br> + chip `[Không cần, tìm ngay]` |
...
| **Bot trả lời** | Summary confirm mới: *"...có: **Wifi, Điều hòa, Bình nóng lạnh**. Tìm ngay nhé?"* <br> + chips `[Tìm ngay]` `[Sửa lại]` |
...
| **Bot trả lời** | *"Mình tìm được **N phòng** phù hợp tại Hồ Chí Minh ạ. Bạn xem thử nhé!"* <br> + kết quả dạng thẻ (tên, địa chỉ, giá rẻ nhất, tiện ích) |
...
| **Bot trả lời** | *(Gemini trả lời dựa trên cached results, không query DB)* <br> *"Phòng số [X] – [Tên phòng] có diện tích lớn nhất với [Y] m²..."* |
...
| **Bot trả lời** | *"Bạn có thể xem chi tiết phòng này tại đây ạ!"* + link đến trang post |
...
| **Bot trả lời** | *(Re-query với budget 5M, bỏ amenity Điều hòa)* <br> *"Mình tìm được **M phòng** sau khi cập nhật yêu cầu ạ."* + kết quả mới |
...
| **Bot trả lời** | *(Session reset → re-process cùng tin nhắn qua SLOT_FILLING)* <br> → Gemini extract: province=BD, placeName="Làng Đại Học", budget=1,500,000 <br> → `count ≤ 5` (BD chỉ có 4 phòng test) → **bỏ qua REFINING** → query trực tiếp <br> *"Mình tìm được **K phòng** phù hợp tại Bình Dương ạ. Bạn xem thử nhé!"* + kết quả |
...
| **Bot trả lời** | *"Cảm ơn bạn đã sử dụng dịch vụ. Chúc bạn tìm được phòng ưng ý ạ!"* |
...
| **Bot trả lời** | *"Mình tìm thấy **~14 phòng** ở Hồ Chí Minh ạ. Bạn có muốn thêm tiện ích (máy lạnh, wifi...) hoặc muốn gần chợ, công viên, trường học không ạ?"* <br> + chip `[Không cần, tìm ngay]` |
...
| **Bot trả lời** | Summary: *"Mình sẽ tìm nhà trọ tại Hồ Chí Minh, ngân sách 2 triệu/tháng. Tìm ngay nhé?"* <br> + chips `[Tìm ngay]` `[Sửa lại]` |
...
|---|---|
| **User click chip** | `Tìm ngay` |
| **Bot trả lời** | *"Mình tìm được **N phòng** phù hợp tại Hồ Chí Minh ạ."* + kết quả |
...
| | |
|---|---|
| **User gõ** | `thêm điều hòa, wifi, bình nóng lạnh, tủ quần áo, giường nệm, camera` *(yêu cầu nhiều tiện ích cùng lúc)* |
...
| **Bot trả lời** | *(Nếu Tier 2/3 có kết quả)* *"Mình tìm được **M phòng** sau khi điều chỉnh yêu cầu ạ."* + kết quả <br> *(Nếu không có gì)* *"Hiện tại chưa tìm thấy phòng phù hợp... Bạn có muốn điều chỉnh ngân sách hoặc khu vực không?"* |
...
| 6 | Validate `provinceCode` (bỏ code không hợp lệ) | *(ngầm)* | — |
...
| 19 | Validate `amenityIds` từ Gemini (bỏ ID không tồn tại) | *(ngầm)* | — |
| 20 | Validate `categoryId` từ Gemini refine | *(ngầm)* | — |
...
| 25 | Rate limit: ≥20 req / 10 phút → báo lỗi | *(edge case)* | — |
| 26 | Bot lỗi kỹ thuật → graceful error message | *(edge case)* | — |
